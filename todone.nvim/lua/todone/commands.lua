...
function M.add_todo()
	vim.ui.input({ prompt = "Todo: " }, function(text)
		...
		pick_project(function(project)
			local name = project.choice.name
			local cmd = string.format("todone add %s --project %s", vim.fn.shellescape(text), vim.fn.shellescape(name))
			...
			if require("todone")._opts.open_after_add then
				...
				vim.notify(string.format('Added "%s" → %s', text, name), vim.log.levels.INFO)
			end
		end)
...
M.pick_project = function()
	pick_project(function(project)
		...
		vim.api.nvim_win_set_cursor(0, { project.choice.lnum, 0 })
	end)
end
